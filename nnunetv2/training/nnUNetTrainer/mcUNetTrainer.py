...
class MultiClassDeepSupervisionWrapper(nn.Module):
    ...
    def forward(self, *args):
        ...
        for i in range(len(labels_per_image_list)):
            if (labels_per_image_list[i] >= class_ind_start_end[0][1]).any():
        ...
        for labels_per_image in labels_per_image_list:
            for dataset_ind, labels_description in enumerate( self.dataset_labels_description):
                if ((labels_description[0] <= labels_per_image).all() and (labels_per_image < class_ind_start_end[1]).all()):
        ...
        for image_ind,dataset_ind in enumerate( dataset_ind_list):
            ...
            for i in range(len(input)):
                output[i] = output[i][image_ind:image_ind+1, dataset_ind_first_label_ind:dataset_ind_last_label_ind]
                target[i] = torch.clamp(target[i][image_ind:image_ind+1] - dataset_ind_first_label_ind, 0,dataset_ind_last_label_ind)
